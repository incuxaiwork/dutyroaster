...
const DUTY_TYPES = [
  "Collector Banglow Guard", "Police Guest House Guard for DIG ATP",
  "S.P Camp Office Guard and Night Guard", "Armoury Guard (Bugler HC-01)",
  "DPO Main Gate Guard", "Treasury Guard Collectorate at ATP",
...
const empty = {
  duty_name: "", duty_type: DUTY_TYPES[0], location: "",
  start_date: today, end_date: "",
  start_time: "08:00", end_time: "14:00", shift_type: "Morning",
  required_officers: 1, required_rank: "", required_skills: "",
  priority_level: "Medium", special_instructions: "", status: "Pending Allocation",
  incharge_officer_id: "" as string | number,
};

function dutyToForm(d: Duty) {
  return {
    duty_name: d.duty_name, duty_type: d.duty_type, location: d.location ?? "",
    start_date: d.start_date ?? today, end_date: d.end_date ?? "",
    start_time: (d.start_time ?? "08:00:00").slice(0, 5),
    end_time: (d.end_time ?? "14:00:00").slice(0, 5),
    shift_type: d.shift_type,
    required_officers: d.required_officers,
    required_rank: d.required_rank ?? "",
    required_skills: Array.isArray(d.required_skills)
      ? d.required_skills.join(", ")
      : String(d.required_skills ?? ""),
    priority_level: d.priority_level,
    special_instructions: d.special_instructions ?? "",
    status: d.status,
    incharge_officer_id: (d.incharge_officer_id ?? "") as string | number,
  };
}
...
export function Duties() {
  ...
  const [allocResult, setAllocResult] = useState<{
    batch_id: number; assigned_count: number; fairness_score: number;
    assignments: Assignment[]; unfilled: unknown[];
  } | null>(null);
  ...
  const filteredOfficers = officers.filter((o) =>
    `${o.name} ${o.rank} ${o.belt_number}`.toLowerCase().includes(officerSearch.toLowerCase())
  );
  const rankOptions = useMemo(() => [...new Set(officers.map((o) => o.rank).filter(Boolean))].sort(), [officers]);

  const load = () =>
    api.duties()
      .then((data) => setDuties(data.filter((d) => d.status !== "Cancelled")))
      .catch((err) => console.error("Failed to load duties:", err));

  useEffect(() => {
    void load();
    api.officers().then(setOfficers).catch(console.error);
  }, []);

  /* ── selection ── */
  const allSelected = duties.length > 0 && selectedIds.size === duties.length;
  function toggleAll()        { setSelectedIds(allSelected ? new Set() : new Set(duties.map((d) => d.id))); }
  ...
  /* ── open form ── */
  function openCreate() {
    setForm(empty); setEditingId(null); setShowForm(true); setError(""); window.scrollTo(0, 0);
  }
  function openEdit(d: Duty) {
    setForm(dutyToForm(d)); setEditingId(d.id); setShowForm(true); setError(""); window.scrollTo(0, 0);
  }
  ...
  async function submit(e: FormEvent) {
    ...
    const payload = {
      ...form,
      required_officers: Number(form.required_officers),
      required_skills: form.required_skills.split(",").map((s) => s.trim()).filter(Boolean),
      incharge_officer_id: form.incharge_officer_id !== "" ? Number(form.incharge_officer_id) : null,
    } as any;
    try {
      if (editingId !== null) {
        await api.updateDuty(editingId, payload);
      } else {
        await api.createDuty(payload);
      }
      closeForm(); load();
  ...
  function deleteDuty(id: number) {
    ...
    api.deleteDuty(id).catch((e) => { setError(String(e)); load(); });
  }

  /* ── generate roster for selected duties ── */
  async function generateRoster() {
    setAllocating(true); setAllocResult(null); setError("");
    try {
      const result = await api.generateRoster({ date: allocDate, duty_ids: [...selectedIds] });
  ...
  const columns = useMemo<ColumnDef<Duty>[]>(() => [
    {
      ...
      cell: ({ row }) => (
        <input type="checkbox" checked={selectedIds.has(row.original.id)} onChange={() => toggleOne(row.original.id)}
          className="h-4 w-4 accent-orange-500 cursor-pointer" />
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${PRIORITY_BADGE[row.original.priority_level] ?? ""}`}>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button size="sm" variant="ghost" title="Edit duty" onClick={() => openEdit(row.original)}>
          <Pencil className="h-4 w-4 text-stone-400 hover:text-orange-500" />
        </Button>
      ),
    },
    {
      ...
      {selectedIds.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-orange-200 bg-orange-50 px-4 py-3">
          ...
          <span className="text-sm font-semibold text-orange-800">
            {selectedIds.size} {selectedIds.size === 1 ? "duty" : "duties"} selected
          </span>
          <div className="flex items-center gap-2 ml-auto flex-wrap">
            <span className="text-xs text-stone-500">For date:</span>
            <Input type="date" value={allocDate} onChange={(e) => setAllocDate(e.target.value)} className="w-40 h-8 text-sm" />
            <Button variant="primary" size="sm" onClick={generateRoster} disabled={allocating}>
              {allocating
                ? <><Loader2 className="h-4 w-4 animate-spin" /> Generating…</>
                : <><Shuffle className="h-4 w-4" /> Generate Roster</>}
            </Button>
      ...
      {allocResult && (
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-green-700 text-base">
                ✅ Roster generated — {allocResult.assigned_count} officer{allocResult.assigned_count !== 1 ? "s" : ""} assigned &nbsp;·&nbsp; Fairness: {allocResult.fairness_score}
              </CardTitle>
              <div className="flex items-center gap-2">
                <Button
                  size="sm"
                ...
                >
                  <Eye className="h-4 w-4" /> View Daily Roster
                </Button>
                <button onClick={() => setAllocResult(null)} className="text-stone-400 hover:text-stone-600 ml-1">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
          ...
          <CardContent className="pt-0">
            {allocResult.unfilled.length > 0 && (
              <p className="mb-3 text-sm text-red-600 font-medium">
                ⚠ {allocResult.unfilled.length} duty/duties could not be fully staffed (insufficient available officers).
              </p>
            )}
            <div className="overflow-x-auto rounded-lg border border-green-200">
              <table className="w-full text-sm">
                <thead className="bg-green-50">
                  <tr>
                    {["Duty", "Location", "Officer", "Belt No.", "Shift", "Hours"].map((h) => (
                      <th key={h} className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-green-700">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {allocResult.assignments.map((a) => (
          ...
          <CardContent>
            ...
            <form onSubmit={submit} className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-stone-500">Duty Name *</label>
                <Input required placeholder="e.g. Market Patrol" value={form.duty_name} onChange={(e) => setForm({ ...form, duty_name: e.target.value })} />
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-stone-500">Duty Type</label>
                <Select value={form.duty_type} onChange={(e) => setForm({ ...form, duty_type: e.target.value })}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input placeholder="e.g. Central Market" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} />
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <Select value={form.shift_type} onChange={(e) => setForm({ ...form, shift_type: e.target.value })}>
                  <option>Morning</option><option>Evening</option><option>Night</option><option>Custom</option>
                </Select>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-stone-500">Officers Required</label>
                <Input type="number" min={1} value={form.required_officers} onChange={(e) => setForm({ ...form, required_officers: Number(e.target.value) })} />
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-stone-500">Required Rank</label>
                <Select value={form.required_rank} onChange={(e) => setForm({ ...form, required_rank: e.target.value })}>
              ...
              <div className="flex flex-col gap-1" ref={officerRef}>
                ...
                <div className="relative">
                  <div
                    className="flex h-10 w-full items-center justify-between rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm cursor-pointer"
                    onClick={() => { setOfficerOpen(!officerOpen); if (!officerOpen) setOfficerSearch(""); }}
                  >
                  ...
                  {officerOpen && (
                    <div className="absolute z-10 top-full mt-1 left-0 right-0 rounded-lg border border-stone-200 bg-white shadow-lg">
                      <input
                        ...
                        className="w-full border-b border-stone-200 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-200"
                        value={officerSearch}
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {filteredOfficers.slice(0, 100).map((o) => (
                          <button
                            key={o.id}
                            type="button"
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-orange-50 ${String(form.incharge_officer_id) === String(o.id) ? "bg-orange-50 font-medium text-orange-700" : ""}`}
                            onClick={() => { setForm({ ...form, incharge_officer_id: o.id }); setOfficerOpen(false); setOfficerSearch(""); }}
                          >
                            {o.name} <span className="text-stone-400">— {o.rank} ({o.belt_number})</span>
                          </button>
                        ))}
                        {filteredOfficers.length === 0 && (
                          <div className="px-3 py-3 text-sm text-stone-400 text-center">No officers found</div>
                        )}
                      </div>
                    </div>
                  )}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-stone-500">Special Instructions</label>
                <Textarea placeholder="Any special instructions…" value={form.special_instructions} onChange={(e) => setForm({ ...form, special_instructions: e.target.value })} />
              </div>

              <div className="md:col-span-2 lg:col-span-3 flex gap-3">
                <Button type="submit" className="flex-1">
                  {editingId !== null ? "Update Duty" : "Create Duty"}
                </Button>
